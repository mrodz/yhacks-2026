...
public enum PromptIteration {
    ...
    V2("""
            ...
            Written textual labels and descriptions must be written in {{LANGUAGE}}
    ...
    V3("""
            ...
            - eta.minutes: integer
            - eta.basis: short explanation of why this ETA was chosen
            - step0.items: (EITHER concrete things the user may need before starting, but only if supported by the document type/content) OR (important caveats and clauses)
            - steps[].description: concise, plain-language, user-facing
            - steps[].targetElementIds: must only contain IDs that appear in the input
            - steps[].pageHint: page number of the first relevant target element
            - steps[].readingOrderHint:
    ...
    public static PromptIteration LATEST = V3;

    PromptIteration(String prompt) {
        this.prompt = prompt;
    }

    public String getPrompt(String language) {
        return this.prompt.replaceAll("\\{\\{LANGUAGE\\}\\}", language);
    }
}
